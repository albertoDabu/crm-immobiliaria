...
// .wrangler/tmp/bundle-d7iLyW/checked-fetch.js
...
var corsHeaders = {
  ...
  // En producción, cambiar a tu dominio de Pages
...
async function createContact(request, env, user) {
  ...
  const { data: contact, error: contactError } = await supabase.from("contacts").insert({
    ...
    notes: body.notes || "",
    registration_date: body.registrationDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
  }).select().single();
  if (contactError) {
...
async function updateContact(request, env, user, contactId) {
  ...
  if (body.contact2) {
    const { error: updateError } = await supabase.from("secondary_contacts").update({
      name: body.contact2.name,
      phone: body.contact2.phone,
      email: body.contact2.email,
      relation: body.contact2.relation
...
async function addContactHistory(request, env, user, contactId) {
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
  const body = await request.json();
  const { data: contact } = await supabase.from("contacts").select("id").eq("id", contactId).eq("user_id", user.id).single();
  if (!contact) {
    return jsonResponse({ error: "Contact not found" }, 404);
  }
  const { data, error } = await supabase.from("contact_history").insert({
    contact_id: contactId,
...
__name(addContactHistory, "addContactHistory");
async function updateContactHistory(request, env, user, contactId, historyId) {
  ...
  const { error } = await supabase.from("contact_history").update({
    note: body.note,
    channel: body.channel,
    feedback: body.feedback
  }).eq("id", historyId).eq("contact_id", contactId);
  if (error) {
    return jsonResponse({ error: error.message }, 500);
  }
  return jsonResponse({ success: true });
}
__name(updateContactHistory, "updateContactHistory");
async function deleteContactHistory(request, env, user, contactId, historyId) {
  ...
  const { error } = await supabase.from("contact_history").delete().eq("id", historyId).eq("contact_id", contactId);
  if (error) {
    return jsonResponse({ error: error.message }, 500);
  }
  return jsonResponse({ success: true });
}
__name(deleteContactHistory, "deleteContactHistory");
async function bulkContact(request, env, user) {
  ...
  const { error } = await supabase.from("contact_history").insert(historyEntries);
  if (error) {
    return jsonResponse({ error: error.message }, 500);
  }
  return jsonResponse({ success: true, contacted: contactIds.length });
}
__name(bulkContact, "bulkContact");
var worker_default = {
...
// .wrangler/tmp/bundle-d7iLyW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-d7iLyW/middleware-loader.entry.ts
